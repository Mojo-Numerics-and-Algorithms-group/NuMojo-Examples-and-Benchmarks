...
# alias size = 1000
fn bench[func: fn() capturing -> None,dtype:DType=DType.float32](owned max_runs:Int=500,max_time_ns:Int=60*10**9, min_runs: Int=25)raises->String:
    # var memory_scaling_array:numojo.NDArray[numojo.f64]=numojo.NDArray[numojo.f64](500)
    # var tens1:NDArray[dtype] = NDArray[dtype](size,size)
    
    # fn trivial_memory()capturing:
    #     var res = tens1
    #     keep(res.unsafe_ptr())
    ...
    #     trivial_memory()
    #     var t2 = now()
    #     memory_scaling_array[i] = t2-t1
    # var mean_memory_cos_ns:Float64 = numojo.cummean(memory_scaling_array)
    var load_scaling_array:numojo.NDArray[numojo.f64]=numojo.NDArray[numojo.f64](min_runs)
    for i in range(min_runs):
        var t1:Float64 = now()
        func()
        var t2:Float64 = now()
        load_scaling_array[i]= t2-t1
     
    if numojo.cummean(load_scaling_array) > max_time_ns:
        return '{"Number of runs": ' + str(min_runs) + ',"mean": '+ str(numojo.cummean(load_scaling_array))+',"std": '+str(numojo.statistics.cumpstdev(load_scaling_array))+'}'
    elif numojo.cummean(load_scaling_array)*max_runs > max_time_ns:
        max_runs = int(numojo.cummean(load_scaling_array)// max_time_ns)
    var array:numojo.NDArray[numojo.f64]=numojo.NDArray[numojo.f64](max_runs)
    for i in range(max_runs):
        var t1:Float64 = now()
        func()
        var t2:Float64 = now()
        array[i]= t2-t1
    return '{"Number of runs": ' + str(max_runs) + ',"mean": '+ str(numojo.cummean(array))+',"std": '+str(numojo.statistics.cumpstdev(array))+'}'


def main():
    # var tens1:NDArray[DType.float32] = NDArray[DType.float32](size,size)
    # var tens2:NDArray[DType.float32] = NDArray[DType.float32](size,size)
    # var tens3:NDArray[DType.float32] = NDArray[DType.float32](size,size)
    print('{')
    for i in range(6,11):
        var size =2**(i)
        var tens1:NDArray[DType.float32] = NDArray[DType.float32](size,size)
        ...
        print('"Mul": ',bench[test_mul](),',')

        fn test_matmul()capturing:
            try:
                var res = numojo.math.linalg.matmul_tiled_unrolled_parallelized(tens1, tens2)
                keep(res.unsafe_ptr())
            except:
                print("mod: Failed shape error")
        print('"Matmul tup": ',bench[test_matmul](),',')
        
        var arr = numojo.arange[numojo.f32,numojo.f32](0,size*size)
        arr.reshape(size,size)
        var arr_f = numojo.NDArray[numojo.f32](size,size,order='F')
        for i in range(size*size):
            arr_f[i]=i
            
        fn test_matmulcf()capturing:
            try:
                var res = numojo.math.linalg.matmul_tiled_unrolled_parallelized(arr, arr_f)
                keep(res.unsafe_ptr())
            except:
                print("matmulcf: Failed shape error")
        print('"Matmul tup CxF": ',bench[test_matmulcf]())
